...
public class Bezier extends PApplet {
  ...
  public void setup()
  {
    ...
    spline.addPoint(new Vector2(100.0f, 100.0f));
    spline.addPoint(new Vector2(300.0f, 300.0f));
    splines.add(spline);
    drawSplines();
  }
  ...
  public void drawSplines(){
    ...
    for(BezierCurve spline : splines){
      ...
      for(float i=0; i<=1; i+= 1.0f/pointsPerSpline){
  ...
  public void interlopePoint(float x, float y){
    for(BezierCurve spline : splines){
      for(int i = 0;i < spline.size();i++){
        Vector2 point = spline.getWaypoint(i);
        if(Math.sqrt(Math.pow(point.x - x, 2) + Math.pow(point.y - y, 2)) < hoverDistance){
  ...
  public boolean isInline(Vector2 point, Vector2 point1, Vector2 point2){
